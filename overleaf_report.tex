...
\texttt{RandomForestClassifier(n\_estimators=100, random\_state=42)} on all 14 diagnostic
features to obtain Gini-impurity-based importance scores (Fig.~\ref{fig:feat_imp}).
The candidate features are:
\texttt{RANGE}, \texttt{FP\_IDX}, \texttt{FP\_AMP1}, \texttt{FP\_AMP2},
\texttt{FP\_AMP3}, \texttt{STDEV\_NOISE}, \texttt{CIR\_PWR}, \texttt{MAX\_NOISE},
\texttt{RXPACC}, \texttt{CH}, \texttt{FRAME\_LEN}, \texttt{PREAM\_LEN},
\texttt{BITRATE}, \texttt{PRFR}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{fig0.png}
    \caption{Gini-impurity feature importance scores for all 14 diagnostic features,
             ranked by a preliminary Random Forest Classifier
             (\texttt{n\_estimators=100, random\_state=42}).
             \texttt{RXPACC} dominates ($\approx$0.30), followed by \texttt{RANGE}
             and \texttt{CIR\_PWR} ($\approx$0.14--0.15). Configuration constants
             \texttt{CH}, \texttt{BITRATE}, \texttt{PRFR}, \texttt{PREAM\_LEN}, and
             \texttt{FRAME\_LEN} contribute near-zero importance and are discarded.}
    \label{fig:feat_imp}
\end{figure}

As shown in Fig.~\ref{fig:feat_imp}, \texttt{RXPACC} is the single most
discriminative feature ($\approx$0.30 importance score). This is physically
intuitive: \texttt{RXPACC} counts the number of preamble symbols accumulated by
the receiver, which is directly affected by the signal-to-noise ratio. Under LOS
conditions the strong direct path allows the receiver to accumulate more symbols,
yielding a higher \texttt{RXPACC} value, whereas NLOS obstruction reduces preamble
accumulation. The measured range (\texttt{RANGE}, $\approx$0.15) and total CIR
power (\texttt{CIR\_PWR}, $\approx$0.14) are the next strongest predictors,
reflecting the systematic positive-bias and energy-spreading effects of NLOS
multipath. The three first-path amplitude estimates (\texttt{FP\_AMP3},
\texttt{FP\_AMP2}, \texttt{FP\_AMP1}) and noise statistics (\texttt{MAX\_NOISE},
\texttt{STDEV\_NOISE}) each contribute moderately, while \texttt{FP\_IDX} provides
a small but non-negligible signal about first-path arrival timing.

In contrast, the five configuration constants \texttt{CH}, \texttt{BITRATE},
\texttt{PRFR}, \texttt{PREAM\_LEN}, and \texttt{FRAME\_LEN} all register
near-zero importance. This is expected: these parameters are fixed hardware
settings that do not vary meaningfully across measurements in this dataset and
therefore carry no discriminative information about LOS versus NLOS propagation.
They are discarded, leaving the following nine predictive features:

\begin{center}
\small
\begin{tabular}{lll}
\toprule
\textbf{Rank} & \textbf{Feature} & \textbf{Meaning} \\
\midrule
1 & \texttt{RXPACC}       & Preamble accumulation count \\
2 & \texttt{RANGE}        & Time-of-flight range estimate \\
3 & \texttt{CIR\_PWR}    & Channel impulse response power \\
4 & \texttt{FP\_AMP3}    & First-path amplitude (3rd estimate) \\
5 & \texttt{MAX\_NOISE}  & Maximum noise sample in CIR \\
6 & \texttt{FP\_AMP2}    & First-path amplitude (2nd estimate) \\
7 & \texttt{FP\_AMP1}    & First-path amplitude (1st estimate) \\
8 & \texttt{STDEV\_NOISE}& Standard deviation of noise floor \\
9 & \texttt{FP\_IDX}     & First-path index in CIR array \\
\bottomrule
\end{tabular}
\end{center}
...
\subsubsection{Multi-Layer Perceptron (MLP) Classifier}
As a comparative solution, a supervised Multi-Layer Perceptron (MLP) was implemented. 

The architecture consists of an input layer (9 features), two hidden layers with 64 
and 32 neurons respectively, and a binary output layer using a Softmax activation 
function. We utilised the 'Adam' optimiser and Rectified Linear Unit (ReLU) 
activation to capture the non-linear signal patterns identified in the 
feature-importance stage. Training was conducted over 500 iterations using the 
same 80:20 stratified split to ensure a fair comparison with the ensemble method.
...
Fig.~\ref{fig:reg1} shows the corresponding Actual vs.\ Predicted scatter. The wider
...
\subsection{Algorithm Comparison: Random Forest vs. MLP}
While the MLP represents a more sophisticated neural network approach, it achieved  a test accuracy of \textbf{81.39\%}, significantly lower than the Random Forest's 
91.40\%. As illustrated in the MLP confusion matrix (Fig. 4), the model exhibited 
a high number of False Negatives (1,451), frequently misclassifying NLOS signals 
as LOS. 

The superior performance of the Random Forest can be attributed to the nature of 
UWB diagnostic features; ensemble tree methods are inherently better at 
identifying discrete physical thresholds in signal power (e.g., \texttt{CIR\_PWR} 
drops) compared to the weight-based linear transformations in a shallow MLP. 
Consequently, the Random Forest remains the primary choice for our proposed 
positioning solution due to its higher reliability in detecting obstacles.

\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{fig4.png}
    \caption{Fig. 4. Confusion Matrix for Multi-Layer Perceptron (MLP) Classifier. While the model shows high sensitivity for LOS signals, the significant number of False Negatives (NLOS misclassified as LOS) indicates a lower reliability for safety-critical indoor positioning compared to the Random Forest approach. }
    \label{fig:placeholder}
\end{figure}
...
\begin{itemize}
    \item \textbf{MLP / deep learning comparison.} The current MLP classifier exhibited a high false-negative rate (1,451 samples), indicating that the model often misidentifies NLOS signals as LOS [cite: 85, 90]. Future work will involve a systematic grid search or Bayesian optimization over learning rates, hidden layer architectures, and dropout regularisation to reduce this bias and match the reliability demonstrated by the Random Forest baseline . 
